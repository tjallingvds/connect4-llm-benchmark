...
1. Push the `tool/` directory to a public GitHub repo.
2. At <https://streamlit.io/cloud>, point a new app at `app.py`.
...
### Hugging Face Spaces
Create a new Space with the **Streamlit** SDK, push these files, and it deploys
on push. Set `app.py` as the entry point.
...
tool/
├── app.py                 # Streamlit UI
├── core.py                # Connect 4 board + minimax oracle
├── llm_clients.py         # OpenAI-compatible + Anthropic transport
├── benchmark.py           # Benchmark runner with progress callbacks
├── results_loader.py      # Loads baseline + community results
├── generated_positions.json  # The 160 benchmark positions
├── baseline_results/      # GPT-4o, Claude 4.5, DeepSeek, Human Expert
├── community_results/     # Runs saved by app users
├── requirements.txt
├── Dockerfile
└── .gitignore
```
...
- The included `.gitignore` excludes `.env`. **Never commit API keys.**
- The deployed app accepts keys at runtime in the sidebar; they live only in
  the Streamlit session, not in logs or disk.
- If you forked from the original repo and your `.env` was ever pushed,
  rotate those keys before deploying.
...
existing `plots_and_stats.py` analysis still works on community runs.
